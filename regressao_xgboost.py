def regressao_xgboost(df): 
     ...
     modelo =  XGBRegressor(n_estimators=100, max_depth =6, learning_rate =0.05, objective ="reg:squarederror")
     modelo.fit(x_treino_scaler,y_treino_scaler.ravel())
     print('Score dados treino: {:.4f}'.format(modelo.score(x_treino_scaler,y_treino_scaler)))
     modelo.fit(x_teste_scaler,y_teste_scaler.ravel())
     print('Score dados teste: {:.4f}'.format(modelo.score(x_teste_scaler,y_teste_scaler)))
 
     previsores_scaler_teste = modelo.predict(x_teste_scaler)
