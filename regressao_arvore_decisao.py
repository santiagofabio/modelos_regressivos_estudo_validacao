def regressao_arvore_decisao(df): 
    ...
    modelo_arvore = DecisionTreeRegressor(max_depth =3,random_state =10)
    modelo_arvore.fit(x_treino_scaler,y_treino_scaler)
    print('Score dados treino: {:.4f}'.format(modelo_arvore.score(x_treino_scaler,y_treino_scaler)))
    modelo_arvore.fit(x_teste_scaler,y_teste_scaler)
    ...
    metricas_erros(y_teste_scaler, previsores_inverse_teste )
